...
def init_db():
    ...
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS admins
                     (user_id INTEGER PRIMARY KEY)''')
        # New table for mandatory channels
...
def load_data():
    ...
    try:
        ...
        conn.close()
        logger.info(f"Data loaded: {len(active_users)} users, {len(user_subscriptions)} subscriptions, {len(admin_ids)} admins, {len(mandatory_channels)} mandatory channels.")
    except Exception as e:
...
def check_code_security(file_path, file_type):
    """فحص الكود بحثاً عن أوامر خطيرة (إصدار مخفف)"""
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read().lower()
        
        # فحص مخفف للأوامر الخطيرة جداً فقط
        dangerous_patterns = [
            'os.system', 'subprocess.popen',  'subprocess','eval', 'exec', 'compile',
            '__import__', 'rm -rf', 'format c:', 'del c:', 'shutdown',
            'reboot', 'chmod 777', 'chmod +x'
        ]
        ...
        if found_patterns:
            ...
            return False, f"الكود يحتوي على أوامر خطيرة: {', '.join(found_patterns)}"
        
        return True, "الكود آمن"
    except Exception as e:
        logger.error(f"Error in security check: {e}")
        return False, f"خطأ في فحص الأمان: {str(e)}"

def scan_zip_security(zip_path):
    """فحص محتويات ZIP لأمانها (إصدار مخفف)"""
    try:
            ...
            for file_info in zip_ref.infolist():
                if file_info.filename.endswith(('.py', '.js')):
                    with zip_ref.open(file_info.filename) as f:
                        ...
                        dangerous_patterns = [
                            'os.system', 'subprocess.popen', 'subprocess', 'eval', 'exec', 
                            'rm -rf', 'format c:', 'shutdown'
                        ]
                        
                        for pattern in dangerous_patterns:
                            if pattern in content:
                                return False, f"الملف {file_info.filename} يحتوي على أمر خطير: {pattern}"
        return True, "الأرشيف آمن"
    except Exception as e:
        return False, f"خطأ في فحص الأرشيف: {str(e)}"

# --- Mandatory Channels Functions ---
def is_user_member(user_id, channel_id):
    """❌ must be join this channel and again try"""
    try:
...
def save_mandatory_channel(channel_id, channel_username, channel_name, added_by):
    """حفظ القناة الإجبارية في قاعدة البيانات"""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
...
def remove_mandatory_channel_db(channel_id):
    """حذف القناة الإجبارية من قاعدة البيانات"""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
...
def create_mandatory_channels_menu():
    """إنشاء قائمة إدارة القنوات الإجبارية"""
    ...
    markup.row(types.InlineKeyboardButton('📋 list channel', callback_data='list_mandatory_channels'))
    markup.row(types.InlineKeyboardButton('🔙 back menu', callback_data='back_to_main'))
    return markup

def create_subscription_check_message(not_joined_channels):
    """إنشاء رسالة التحقق من الاشتراك"""
    message = "📢 **Important Fast Join Now Our Channel And Try Again:**\n\n"
    ...
    for channel_id, channel_info in not_joined_channels:
        ...
        markup.add(types.InlineKeyboardButton(f"انضم إلى {channel_name}", url=channel_link))
    
    markup.add(types.InlineKeyboardButton("✅ تحقق من الاشتراك", callback_data='check_subscription_status'))
...
def create_main_menu_inline(user_id):
    ...
    if user_id in admin_ids:
        ...
        markup.add(admin_buttons[6]) # Channel Management
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        if not is_safe:
            ...
            security_warning_msg = f"🚨 ملف يحتاج موافقة:\n👤 المستخدم: {user_id}\n📁 الملف: {file_name_zip}\n⚠️ السبب: {security_msg}"
            markup = types.InlineKeyboardMarkup()
            markup.row(
                types.InlineKeyboardButton("✅ موافقة", callback_data=f"approve_zip_{user_id}_{file_name_zip}"),
                types.InlineKeyboardButton("❌ رفض", callback_data=f"reject_zip_{user_id}_{file_name_zip}")
            )
            ...
            bot.reply_to(message, f"⏳ الملف تحت المراجعة للأمان. سيتم إعلامك عند الموافقة.")
            return
...
def _logic_manage_mandatory_channels(message):
    """إدارة القنوات الإجبارية - للادمن فقط"""
    if message.from_user.id not in admin_ids:
        bot.reply_to(message, "⚠️ Admin permissions required.")
        return
    bot.reply_to(message, "📢 إدارة القنوات الإجبارية\nاستخدم الأزرار أدناه:", reply_markup=create_mandatory_channels_menu())
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    "📢 إدارة القنوات الإجبارية": _logic_manage_mandatory_channels,  # New button
}
...
def command_run_all_code(message): _logic_run_all_scripts(message)
 # New command for channel management
...
def handle_file_upload_doc(message): # Renamed
    ...
    try:
        ...
        if file_ext == '.zip':
            ...
            # Security check for script files (مخفف)
            ...
            if not is_safe:
                ...
                markup = types.InlineKeyboardMarkup()
                markup.row(
                    types.InlineKeyboardButton("✅ موافقة", callback_data=f"approve_file_{user_id}_{file_name}"),
                    types.InlineKeyboardButton("❌ رفض", callback_data=f"reject_file_{user_id}_{file_name}")
                )
                for admin_id in admin_ids:
                    try:
                        bot.send_message(admin_id, security_warning_msg, reply_markup=markup)
...
def manage_mandatory_channels_callback(call):
    """معالجة طلب إدارة القنوات الإجبارية"""
    bot.answer_callback_query(call.id)
    try:
        bot.edit_message_text("📢 إدارة القنوات الإجبارية\nاختر الإجراء المطلوب:",
                              call.message.chat.id, call.message.message_id, 
...
def add_mandatory_channel_callback(call):
    ...
    bot.answer_callback_query(call.id)
    msg = bot.send_message(call.message.chat.id, "📢 أرسل معرف القناة أو اليوزرنيم (مثال: @channel_username أو -1001234567890)\n/cancel للإلغاء")
...
def process_add_channel(message):
    ...
    if message.text and message.text.lower() == '/cancel':
        bot.reply_to(message, "❌ تم إلغاء إضافة القناة.")
        return
    ...
    try:
        ...
        try:
            ...
            if bot_member.status not in ['administrator', 'creator']:
                bot.reply_to(message, f"❌ البوت ليس أدمن في القناة! يجب ترقيته أولاً.")
                return
        except Exception as e:
            bot.reply_to(message, f"❌ البوت ليس أدمن في القناة أو لا يستطيع الوصول إليها!")
            return
            
        # Save channel to database
        if save_mandatory_channel(channel_id, channel_username, channel_name, admin_id):
            bot.reply_to(message, f"✅ تم إضافة القناة الإجبارية:\n**{channel_name}**\n{channel_username or channel_id}")
        else:
            bot.reply_to(message, "❌ فشل في إضافة القناة. حاول مرة أخرى.")
...
def remove_mandatory_channel_callback(call):
    ...
    markup.add(types.InlineKeyboardButton("🔙 رجوع", callback_data='manage_mandatory_channels'))
    
    try:
        bot.edit_message_text("📢 اختر القناة التي تريد حذفها:",
                              call.message.chat.id, call.message.message_id, 
                              reply_markup=markup)
    except Exception as e:
...
def process_remove_channel(call):
    ...
    if channel_id in mandatory_channels:
        ...
        if remove_mandatory_channel_db(channel_id):
            ...
            try:
                bot.edit_message_text(f"✅ تم حذف القناة الإجبارية: **{channel_name}**",
                                      call.message.chat.id, call.message.message_id,
            ...
            bot.answer_callback_query(call.id, "❌ فشل في حذف القناة.", show_alert=True)
    else:
...
def list_mandatory_channels_callback(call):
    ...
    if not mandatory_channels:
        message_text = "📢 **لا توجد قنوات إجبارية حالياً**"
    else:
        message_text = "📢 **القنوات الإجبارية:**\n\n"
        for channel_id, channel_info in mandatory_channels.items():
            channel_name = channel_info.get('name', 'Unknown')
            channel_username = channel_info.get('username', 'لا يوجد يوزرنيم')
...
def check_subscription_status_callback(call):
    ...
    if is_subscribed or user_id in admin_ids:
        bot.answer_callback_query(call.id, "✅ أنت مشترك في جميع القنوات المطلوبة!", show_alert=True)
        # Show main menu
        try:
        ...
        bot.answer_callback_query(call.id, "❌ لم تنضم بعد لجميع القنوات المطلوبة!", show_alert=True)
...
def process_approve_file(call):
    ...
    try:
        ...
        bot.edit_message_text(f"✅ تمت الموافقة على الملف `{file_name}` للمستخدم `{user_id}`",
                              call.message.chat.id, call.message.message_id)
        
        # Notify user
        try:
            bot.send_message(user_id, f"✅ تمت الموافقة على ملفك `{file_name}` وبدأ التشغيل.")
        except Exception as e:
            logger.error(f"Failed to notify user {user_id}: {e}")
            
    except Exception as e:
        logger.error(f"Error processing approved file: {e}")
        bot.answer_callback_query(call.id, "❌ خطأ في معالجة الملف.", show_alert=True)

def process_reject_file(call):
    """معالجة رفض الأدمن لملف"""
    data_parts = call.data.split('_')
    if len(data_parts) < 4:
        bot.answer_callback_query(call.id, "❌ بيانات غير صالحة.", show_alert=True)
        return
    ...
    bot.edit_message_text(f"❌ تم رفض الملف `{file_name}` للمستخدم `{user_id}`",
                          call.message.chat.id, call.message.message_id)
    
    # Notify user
    try:
        bot.send_message(user_id, f"❌ تم رفض ملفك `{file_name}` لأسباب أمنية.")
    except Exception as e:
        logger.error(f"Failed to notify user {user_id}: {e}")

def process_approve_zip(call):
    """معالجة موافقة الأدمن على ملف ZIP"""
    data_parts = call.data.split('_')
    if len(data_parts) < 4:
        bot.answer_callback_query(call.id, "❌ بيانات غير صالحة.", show_alert=True)
        return
    ...
    # This would need to be implemented to re-process the ZIP file
    # For now, we'll just notify
    bot.answer_callback_query(call.id, "✅ تمت الموافقة على الأرشيف!")
    bot.edit_message_text(f"✅ تمت الموافقة على الأرشيف `{file_name}` للمستخدم `{user_id}`",
                          call.message.chat.id, call.message.message_id)
    
    try:
        bot.send_message(user_id, f"✅ تمت الموافقة على أرشيفك `{file_name}`. يرجى إعادة رفعه.")
    except Exception as e:
        logger.error(f"Failed to notify user {user_id}: {e}")

def process_reject_zip(call):
    """معالجة رفض الأدمن لملف ZIP"""
    data_parts = call.data.split('_')
    if len(data_parts) < 4:
        bot.answer_callback_query(call.id, "❌ بيانات غير صالحة.", show_alert=True)
        return
    ...
    bot.answer_callback_query(call.id, "❌ تم رفض الأرشيف!")
    bot.edit_message_text(f"❌ تم رفض الأرشيف `{file_name}` للمستخدم `{user_id}`",
                          call.message.chat.id, call.message.message_id)
    
    try:
        bot.send_message(user_id, f"❌ تم رفض أرشيفك `{file_name}` لأسباب أمنية.")
    except Exception as e:
        logger.error(f"Failed to notify user {user_id}: {e}")
